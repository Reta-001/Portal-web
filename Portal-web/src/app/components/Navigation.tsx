...
export default function Navigation() {
  ...
  const navItems = [
    ...
    { name: 'Valores y Reservas', href: '/valores-reservas' },
    { name: 'Nosotros', href: '/nosotros' },
    { name: 'Contacto', href: '/contacto' },
    { name: 'cris', href: '/peluqueria' },
  ];
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-gray-700 hover:text-pink-600 px-3 py-2 text-long font-medium transition-colors duration-200"
                >
                  {item.name}
                </Link>
              ))}
